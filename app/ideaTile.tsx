				...
				{opened ? (
					<>
						...
						<motion.div
							...
							className="w-full flex flex-col sm:flex-row gap-x-1 md:gap-x-5 gap-y-1.5 xs:gap-y-2 pt-3 border-t-2 border-Black/50"
						>
							<div className={`flex-1 ${props.idea.done != undefined ? "" : "hidden"}`}>
								...
								<p className={`text-xs md:text-sm md:ps-1 pt-1 ${notoFont400.className}`}>{props.idea.done}</p>
							</div>
							...
							<div className={`flex-1 ${props.idea.currently != undefined ? "" : "hidden"}`}>
								...
								<p className={`text-xs md:text-sm md:ps-1 pt-1 ${notoFont400.className}`}>{props.idea.currently}</p>
							</div>
							...
							<div className={`flex-1 ${props.idea.todo != undefined ? "" : "hidden"}`}>
								...
								<p className={`text-xs md:text-sm md:ps-1 pt-1 ${notoFont400.className}`}>{props.idea.todo}</p>
							</div>
						</motion.div>
					</>
				) : (
